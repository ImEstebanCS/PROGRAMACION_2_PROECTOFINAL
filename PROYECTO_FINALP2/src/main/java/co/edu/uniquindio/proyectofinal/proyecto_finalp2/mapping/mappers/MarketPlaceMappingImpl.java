...
import co.edu.uniquindio.proyectofinal.proyecto_finalp2.factory.ModelFactory;
...
public class MarketPlaceMappingImpl implements IMarketPlaceMapping {
    ...
    public void vincularVendedores(Vendedor vendedor1, Vendedor vendedor2) throws Exception {
        ...
        if (vendedor1.getContactos().contains(vendedor2)) {
            throw new Exception("Los vendedores ya están vinculados");
        }

        if (vendedor1.getContactos().size() >= MarketPlaceConstantes.MAX_CONTACTOS) {
            throw new Exception("El vendedor 1 ha alcanzado el límite de contactos");
        }

        if (vendedor2.getContactos().size() >= MarketPlaceConstantes.MAX_CONTACTOS) {
    ...
    public List<Vendedor> obtenerContactosSugeridos(Vendedor vendedor) {
        ...
        // Obtener contactos de contactos que no estén en la lista actual
    ...
    public List<Producto> obtenerProductosRed(Vendedor vendedor) {
        ...
        // Agregar productos de contactos
